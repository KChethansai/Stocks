...
function XAxis(props) {
  ...
  return /* @__PURE__ */ React.createElement(XAxisInner, { ...props, container });
}
const XAxisInner = memo(function XAxisInner2({
  ...
  // Drop a trailing tick that would collide with the y-axis gutter: the last
  // data tick sits at the plot's right edge and its centered label overlaps
  // the y tick values (the "Sep 12" + "$0" jumble).
  const visibleLabels = useMemo(() => {
    const plotRight = (container?.clientWidth || 0) - (margin?.right ?? 0);
    const filtered = labelsToShow.filter((t) => t.x <= plotRight - 36);
    return filtered.length >= 2 ? filtered : labelsToShow;
  }, [labelsToShow, container, margin]);
  return createPortal(
    /* @__PURE__ */ React.createElement("div", { className: "pointer-events-none absolute inset-0" }, visibleLabels.map((item) => /* @__PURE__ */ React.createElement(
      XAxisLabel,
      {
